...
def read_input_trace(file_name):
    ...
    return cpu_time,memory_requests
...
def generate_dram_commands(memory_requests):
    ...
    for request in memory_requests:
        ...
        if operation == 0 or operation == 2:
            ...
            dram_commands.append(f"{cpu_time} {channel} ACT0 {bank_group} {bank} {row}")
           # dim_time += 2
            dram_commands.append(f"{time} {channel} ACT1 {bank_group} {bank} {row}")
           # dim_time += 2
            dram_commands.append(f"{time} {channel} RD0  {bank_group} {bank} {column}{low_column}")
           # dim_time += 2
            dram_commands.append(f"{time} {channel} RD1  {bank_group} {bank} {column}{low_column}")
            #dim_time += 2
            dram_commands.append(f"{time} {channel} PRE  {bank_group} {bank} \n")
          #  dim_time += 2
        elif operation == 1:
           # print(f"At every cpu cycle time: {time}")
            ...
            time += 3
            dram_commands.append(f"{time} {channel} ACT0 {bank_group} {bank} {row}")
            dram_commands.append(f"{time} {channel} ACT1 {bank_group} {bank} {row}")
           # dim_time += 2
            dram_commands.append(f"{time} {channel} WR0  {bank_group} {bank} {column}{low_column}")
           # dim_time += 4
            dram_commands.append(f"{time} {channel} WR1  {bank_group} {bank} {column}{low_column}")
           # dim_time += 4
            dram_commands.append(f"{time} {channel} PRE  {bank_group} {bank} \n")
           # dim_time += 2
